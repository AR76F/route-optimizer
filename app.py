...
def add_labeled_marker(fmap: folium.Map, lat: float, lon: float, label: str, kind: str):
    ...
    folium.Marker(
        [lat, lon],
        icon=folium.DivIcon(
            icon_size=(260, 22), icon_anchor=(0, -18),
            html=f"""<div style="display:inline-block;padding:2px 6px;font-size:12px;font-weight:700;color:#111;background:rgba(255,255,255,.95);border:1px solid #ddd;border-radius:6px;box-shadow:0 1px 2px rgba(0,0,0,.25);white-space:nowrap;">{label}</div>"""
        ),
    ).add_to(fmap)
def render_page_1():
    cummins_header()
    ...
    with tabs[0]:
        ...
        if geotab_enabled_by_secrets:
                ...
                def fetch_one(did):
                    try:
                        ...
                        if lat is not None and lon is not None:
                            return {"deviceId": did, "lat": float(lat), "lon": float(lon), "when": when, "driverName": driver_name}
            ...
            def _label_for_device(device_id, device_name, driver_from_api):
            ...
            if not devs:
                ...
                for d in devs:
                    ...
                    options.append(lbl)
                    label2id[lbl] = d["id"]
                picked_labels = st.multiselect("Sélectionner un ou plusieurs véhicules/techniciens à afficher :", sorted(options), default=[], key="geo_pick_labels")
                ...
                if wanted_ids:
                    ...
                    if valid:
                        ...
                        for p in valid:
                            ...
                            folium.CircleMarker([p["lat"], p["lon"]], radius=8, color="#222", weight=2, fill=True, fill_color=color, fill_opacity=0.9).add_to(fmap)
                            folium.Marker([p["lat"], p["lon"]], popup=folium.Popup(f"<b>{label}</b><br>Recency: {lab}<br>{p['lat']:.5f}, {p['lon']:.5f}", max_width=320), tooltip=label,
                                icon=folium.DivIcon(icon_size=(240, 22), icon_anchor=(0, -18),
                                html=f"""<div style="display:inline-block;padding:2px 6px;font-size:12px;font-weight:700;color:#111;background:rgba(255,255,255,.95);border:1px solid #ddd;border-radius:6px;box-shadow:0 1px 2px rgba(0,0,0,.25);white-space:nowrap;">{label.split(' — ')[0]}</div>""")
                            ).add_to(fmap)
    ...
    with tabs[1]:
        ...
        tech_home_df = pd.DataFrame([{"tech_name": name, "home_address": addr, "postal": _extract_postal(addr)} for name, addr in TECH_HOME.items()])
        ...
        with c1b:
            tech_choice = st.selectbox("Technicien → définir comme **départ**", ["(choisir)"] + sorted(TECH_HOME.keys()), key="tech_choice_start_tab2")
        ...
        with c2b:
            ent_choice = st.selectbox("Entrepôt → définir comme **stockage**", ["(choisir)"] + sorted(ENTREPOTS.keys()), key="entrepot_choice_storage_tab2")
    ...
    if st.button("🧭 Optimize Route", type="primary", key="optimize_btn"):
        try:
            ...
            directions = gmaps_client.directions(
                ...
                waypoints=wp_arg, departure_time=departure_dt,
            ...
            st.session_state.route_result = {
                ...
                "wp_geocoded": wp_geocoded, "round_trip": st.session_state.get("round_trip", True),
                "overview": directions[0].get("overview_polyline", {}).get("points"), "per_leg": per_leg,
            }
        except Exception as e:
    ...
    if res:
        ...
        round_trip_res = res["round_trip"]; overview = res.get("overview"); per_leg = res.get("per_leg", [])
            ...
            try:
                ...
                if overview:
                    try:
                        ...
                        pass
                folium.Marker(start_ll, icon=folium.Icon(color="green", icon="play", prefix="fa"), popup=folium.Popup(f"<b>START</b><br>{visit_texts[0]}", max_width=260)).add_to(fmap)
                ...
                if not end_ll:
                    ...
                    if g: end_ll = (g[0], g[1])
                if end_ll:
                    folium.Marker(end_ll, icon=folium.Icon(color="red", icon="flag-checkered", prefix="fa"), popup=folium.Popup(f"<b>{'END (Home)' if round_trip_res else 'END'}</b><br>{end_addr}", max_width=260)).add_to(fmap)
                st_folium(fmap, height=800, width=1800)
            except Exception as e:
...
def render_page_2():
    ...
    if tech_df is None or len(tech_df) == 0:
        def _extract_postal(addr: str) -> str:
            ...
            m = re.search(r"\b([A-Z]\d[A-Z])\s?(\d[A-Z]\d)\b", str(addr).upper())
            return (m.group(1) + m.group(2)) if m else ""
        tech_df = pd.DataFrame([{"tech_name": name, "home_address": addr, "postal": _extract_postal(addr)} for name, addr in TECH_HOME.items()])
    ...
    COL_LAST_INSP = pick_col(jobs_raw, ["LAST INSPECTION", "Last Inspection", "LastInspection", "LAST_INSPECTION", "DERNIÈRE INSPECTION", "Derniere inspection", "Dernière inspection"])
    ...
    # ── NOUVEAU : colonne ALL OPEN WORK — lecture statique, zéro appel API ──
    ...
    # ── NOUVEAU : ALL OPEN WORK — simple colonne texte, aucun geocode, aucun API ──
    ...
    def _get_db() -> sqlite3.Connection:
        ...
        conn.execute("CREATE TABLE IF NOT EXISTS travel (k TEXT PRIMARY KEY, minutes INTEGER, ts INTEGER)")
    ...
    def travel_min_estimate(origin_addr, dest_addr, origin_lat=None, origin_lon=None, dest_lat=None, dest_lon=None) -> int:
        try:
            k = _key(origin_addr, dest_addr, use_traffic)
            now = int(time.time())
            min_ts = now - int(cache_days) * 86400
            conn = _get_db()
            row = conn.cursor().execute("SELECT minutes FROM travel WHERE k=? AND ts>=?", (k, min_ts)).fetchone()
            if row:
                return int(row[0])
        except Exception:
            pass
        try:
            ...
            if origin_lat and dest_lat:
                km = haversine_km(float(origin_lat), float(origin_lon), float(dest_lat), float(dest_lon))
    ...
    def prefetch_travel_matrix(origins: List[str], destinations: List[str], progress_cb=None) -> int:
        ...
        CHUNK_DEST = 10
        total_new = 0
        all_orig_chunks = [missing_origins[i:i+CHUNK_ORIG] for i in range(0, len(missing_origins), CHUNK_ORIG)]
        total_calls = 0
        for orig_chunk in all_orig_chunks:
            all_dests_for_chunk = list({d for o in orig_chunk for d in missing_dests_per_origin.get(o, [])})
            dest_chunks = [all_dests_for_chunk[i:i+CHUNK_DEST] for i in range(0, len(all_dests_for_chunk), CHUNK_DEST)]
            for dest_chunk in dest_chunks:
                if not dest_chunk: continue
                try:
                    r = gmaps_client.distance_matrix(orig_chunk, dest_chunk, mode="driving")
                    total_calls += 1
                    rows_data = r.get("rows", [])
                    inserts = []
                    for oi, row_data in enumerate(rows_data):
                        if oi >= len(orig_chunk): break
                        orig = orig_chunk[oi]
                        for di, el in enumerate(row_data.get("elements", [])):
                            if di >= len(dest_chunk): break
                            dest = dest_chunk[di]
                            if orig == dest: continue
                            if dest not in missing_dests_per_origin.get(orig, []): continue
                            ...
                            total_new += 1
                    if inserts:
                        conn.executemany("INSERT OR REPLACE INTO travel(k, minutes, ts) VALUES(?,?,?)", inserts)
                        conn.commit()
                except Exception:
                    pass
                if progress_cb:
    ...
    def haversine_vectorized(tlat, tlon, lats, lons):
        R = 6371.0
        ...
        p1 = np.radians(float(tlat)); p2 = np.radians(lats)
        dp = np.radians(lats - float(tlat)); dl = np.radians(lons - float(tlon))
    ...
    def _ensure_geocode_table():
        conn = _get_db()
        conn.execute("CREATE TABLE IF NOT EXISTS geocode (addr_key TEXT PRIMARY KEY, lat REAL, lon REAL, ts INTEGER)")
        conn.commit()
    ...
    def get_ll_for_address(addr: str) -> Tuple[Optional[float], Optional[float]]:
        ...
        try:
            conn = _get_db()
            conn.execute("INSERT OR REPLACE INTO geocode (addr_key, lat, lon, ts) VALUES (?,?,?,?)", (key, lat, lon, int(__import__("time").time())))
            conn.commit()
        except Exception:
            pass
        return lat, lon

    def classify_sector(lat, lon) -> str:
        if lat is None or lon is None: return "UNK"
        lat = float(lat); lon = float(lon)
        if lat > 46.15: return "ZONE_NORD"
        if lon > -73.15 and 45.25 < lat < 46.10: return "ZONE_EST"
        if lon < -74.00 and lat < 45.60: return "RIVE_SUD_OUEST"
        if lat > 45.70: return "RIVE_NORD"
        if (45.52 <= lat <= 45.70) and (-73.97 <= lon <= -73.52): return "RIVE_NORD"
        if lat >= 45.62 and not (-74.03 <= lon <= -73.38): return "RIVE_NORD"
        if (45.42 <= lat <= 45.70) and (-74.03 <= lon <= -73.38): return "MTL_ILE"
        if lat < 45.55: return "RIVE_SUD"
        return "MTL_ILE"
    ...
    def sector_compatible(tech_sector: str, job_sector: str) -> bool:
        if tech_sector == "UNK" or job_sector == "UNK": return True
    ...
    st.sidebar.caption(
        "Le précalcul des trajets se fait **automatiquement** à chaque nouvel upload. "
    ...
    def ensure_job_ll_master(master_df, master_idx):
        r = master_df.loc[master_idx]
        lat = r.get("job_lat"); lon = r.get("job_lon"); sec = r.get("job_sector", "UNK") or "UNK"
    ...
    def get_job_pool_for_tech(master_remaining, tech_name, pool_size):
        if master_remaining.empty: return master_remaining
    ...
    def rank_techs_for_job(tech_names, job_row, top_n):
        if top_n >= len(tech_names): return tech_names
        ...
        for t in tech_names:
            ...
            scored.append((t, d))
        if not scored: return tech_names[:max(2, int(top_n))]
    ...
    def style_duo(df: pd.DataFrame):
        ...
        def _row_style(row):
            ...
            if ot_val:
                return ["background-color: #f8d7da;color: #000000;font-weight: 900;border-left: 6px solid #b02a37;"] * len(row)
            try:
                n = int(row.get("techs_needed", 1))
            except Exception:
                n = 1
            if n >= 3:
                return ["background-color: #f8d7da;color: #000000;font-weight: 800;border-left: 6px solid #b02a37;"] * len(row)
            if n == 2:
                return ["background-color: #ffd966;color: #000000;font-weight: 700;border-left: 6px solid #ff9800;"] * len(row)
            return [""] * len(row)
        return df.style.apply(_row_style, axis=1)

    INSPECTION_KEYWORDS = ["inspection", "generator inspection", "génératrice inspection", "inspection génératrice"]

    def filter_by_service_type(df, mode_label):
        if mode_label == "Inclure full service (tous les jobs)": return df
        ...
        if mode_label == "Generator inspection seulement": return df[mask].copy()
        if mode_label == "Exclure generator inspection": return df[~mask].copy()
        return df

    def compute_total_parts(job_minutes_total, daily_onsite_cap):
        if daily_onsite_cap <= 0: return 1
        return int(math.ceil(float(job_minutes_total) / float(daily_onsite_cap)))
    # ── _extra_fields_from_job inclut all_open_work ──────────────────
    ...
    def schedule_month_with_duo(jobs_in, tech_names, month_days, day_hours, lunch_min,
                                 buffer_job, max_jobs_per_day, allow_duo,
                                 progress=None, progress_text=None) -> dict:
        ...
        # ── FIX 1 : pré-géocoder tous les jobs UNE SEULE FOIS ──────────────
        # _count_compatible_techs appelle get_ll_for_address par job.
        # Sans ce pré-calcul, elle serait rappelée pour chaque jour ouvrable
        # (ex: 100 jobs × 20 jours = 2000 tentatives de géocodage).
        # Après ce bloc, tous les jobs ont leurs coordonnées dans jobs.index
        # et le cache mémoire ll_cache, donc les appels suivants sont O(1).
        for _pre_idx in remaining_all.index:
            if _pre_idx in jobs.index:
                if pd.isna(jobs.at[_pre_idx, "job_lat"]) or jobs.at[_pre_idx, "job_lat"] is None:
                    ensure_job_ll_master(jobs, _pre_idx)
        # ── FIN FIX 1 ────────────────────────────────────────────────────────

        def _register_and_relabel_split_row(base_job_id, new_row_idx, part_num):
            ...
            if int(part_num) > int(stt["total"]): stt["total"] = int(part_num)
        ...
        def _book_split_part_for_tech(day, t, used, cur_loc, jobs_count, lock_tech, split_state):
            ...
            jlat, jlon = get_ll_for_address(addr)
            jsec = classify_sector(jlat, jlon)
            if not sector_compatible(tsec, jsec): return
            ...
            planned_rows.append({
                ...
                "adresse": addr, "travel_min": int(tmin), "job_min": int(onsite_today),
                "buffer_min": int(buffer_job), "techs_needed": techs_needed_val,
                "last_inspection": split_state.get("last_inspection", ""),
                "difference": split_state.get("difference", ""),
                "unit": split_state.get("unit", ""),
                "serial_number": split_state.get("serial_number", ""),
                "all_open_work": split_state.get("all_open_work", ""),
                "description": desc,
            })
        ...
        def _sort_techs_by_proximity(tech_list, remaining_jobs):
            if remaining_jobs.empty: return tech_list
            scores = []
            for t in tech_list:
                tlat, tlon = tech_ll_map.get(t, (None, None))
                if tlat is None or tlon is None:
                    scores.append((t, float('inf'))); continue
                tsec = _tech_sector.get(t, "UNK")
                best_dist = float('inf')
                for idx, job in remaining_jobs.iterrows():
                    ...
                    if jlat is None or pd.isna(jlat): continue
                    jsec = jobs.at[idx, "job_sector"] if idx in jobs.index and "job_sector" in jobs.columns else "UNK"
                    if not sector_compatible(tsec, jsec or "UNK"): continue
        ...
        for di, day in enumerate(month_days):
            ...
            solo_jobs = remaining_all[remaining_all["techs_needed"] <= 1].copy()
            solo_jobs = solo_jobs[~solo_jobs["job_id"].apply(normalize_base_job_id).isin(planned_base_ids)]
            try:
                solo_jobs["_n_techs_compat"] = solo_jobs.apply(_count_compatible_techs, axis=1)
                solo_jobs = solo_jobs.sort_values(["_n_techs_compat", "job_id"], kind="mergesort")
                solo_jobs = solo_jobs.drop(columns=["_n_techs_compat"])
            ...
            duo_jobs = remaining_all[remaining_all["techs_needed"] == 2].copy() if allow_duo else remaining_all.iloc[0:0].copy()
            duo_jobs = duo_jobs[~duo_jobs["job_id"].apply(normalize_base_job_id).isin(planned_base_ids)]
            ...
            if allow_duo and (not duo_jobs.empty) and len(tech_names) >= 2:
                while True:
                    ...
                    for jidx, job in sample.iterrows():
                        ...
                        for i in range(len(near_techs)):
                            for k in range(i + 1, len(near_techs)):
                                t1 = near_techs[i]; t2 = near_techs[k]
                                if lock_tech.get(t1) or lock_tech.get(t2): continue
                                if jobs_count[t1] >= int(max_jobs_per_day) or jobs_count[t2] >= int(max_jobs_per_day): continue
                                if not sector_compatible(_tech_sector.get(t1, "UNK"), jsec): continue
                                if not sector_compatible(_tech_sector.get(t2, "UNK"), jsec): continue
                                ...
                                if (end_m + int(t1_back) <= available) and (end_m + int(t2_back) <= available):
                                    ...
                                    if best is None or score < best[0]:
                                        best = (score, jidx, t1, t2, start_m, end_m, int(t1_tr), int(t2_tr), duo_is_overtime, int(job_min_each))
                    ...
                    for tname, trv in [(t1, t1_tr), (t2, t2_tr)]:
                        jobs_count[tname] += 1
                        planned_rows.append({
                            "date": day.isoformat(), "technicien": tname, "sequence": jobs_count[tname],
                            "job_id": job["job_id"], "cust": job.get("cust", ""),
                            "duo": "⚠️ DUO", "ot": "",
                            "debut": mm_to_hhmm(int(start_m)), "fin": mm_to_hhmm(int(end_m)),
                            "adresse": job["address"], "travel_min": int(trv),
                            "job_min": int(job_min_each), "buffer_min": int(buffer_job),
                    ...
                    if bool(duo_is_overtime):
                        lock_tech[t1] = True; lock_tech[t2] = True
            ...
            if not solo_jobs.empty:
                ...
                while made_progress:
                    ...
                    for t in _ordered_techs:
                        if solo_jobs.empty: break
                        if lock_tech.get(t, False): continue
                        if jobs_count[t] >= int(max_jobs_per_day): continue
                        if booked_ids:
                            solo_jobs = solo_jobs[~solo_jobs["job_id"].isin(booked_ids)]
                            booked_ids.clear()
                            if solo_jobs.empty: break
                        best_idx = None; best_cost = None; best_tmin = None
                        sample = get_job_pool_for_tech(solo_jobs, t, int(solo_pool))
                        for idx, job in sample.iterrows():
                            if normalize_base_job_id(str(job["job_id"])) in planned_base_ids: continue
                            jlat, jlon, jsec = ensure_job_ll_master(jobs, idx) if idx in jobs.index else (_jll := get_ll_for_address(job.get("address","")), _jll[0], _jll[1], classify_sector(_jll[0], _jll[1]))[1:]
                            ...
                            if used[t] + need <= available:
                                if best_cost is None or int(tmin) < best_cost:
                                    best_idx = idx; best_cost = int(tmin); best_tmin = int(tmin)
                        if best_idx is not None:
                            ...
                            if best_tmin_real >= 9999: best_tmin_real = best_tmin
                            ...
                            planned_rows.append({
                                ...
                                "duo": "", "ot": "",
                                "debut": mm_to_hhmm(int(start_m)), "fin": mm_to_hhmm(int(end_m)),
                                "adresse": job["address"], "travel_min": int(best_tmin_real),
                            ...
                            planned_base_ids.add(normalize_base_job_id(job["job_id"]))
                            used[t] = int(end_m); cur_loc[t] = job["address"]
                            booked_ids.add(job["job_id"]); made_progress = True
                            ...
                            if _booked_cust and not solo_jobs.empty and "cust" in solo_jobs.columns:
                                ...
                                if _same_cust.any():
                                    solo_jobs = pd.concat([solo_jobs[_same_cust], solo_jobs[~_same_cust]])
                            continue
                        if jobs_count[t] == 0:
                            best_ot_idx = None; best_ot_cost = None; best_ot_tmin = None
                            for idx, job in sample.iterrows():
                                if normalize_base_job_id(str(job["job_id"])) in planned_base_ids: continue
                                jlat, jlon, jsec = ensure_job_ll_master(jobs, idx) if idx in jobs.index else (_jll := get_ll_for_address(job.get("address","")), _jll[0], _jll[1], classify_sector(_jll[0], _jll[1]))[1:]
                                if not sector_compatible(_tech_sector.get(t, "UNK"), jsec): continue
                                ...
                                if need <= OT_ACTIVE_CAP:
                                    if best_ot_cost is None or int(tmin) < best_ot_cost:
                                        best_ot_idx = idx; best_ot_cost = int(tmin); best_ot_tmin = int(tmin)
                            if best_ot_idx is not None:
                                ...
                                planned_base_ids.add(normalize_base_job_id(job["job_id"]))
                                used[t] = int(end_m); cur_loc[t] = job["address"]
                                booked_ids.add(job["job_id"]); lock_tech[t] = True; made_progress = True
                                continue
                        best_long_idx = None; best_long_cost = None; best_long_is_overtime = False
                        for idx, job in sample.iterrows():
                            jm = int(job["job_minutes"])
                            if jm <= int(daily_onsite_cap): continue
                            if t in carryover_by_tech: continue
                            if normalize_base_job_id(str(job["job_id"])) in planned_base_ids: continue
                            jlat, jlon, jsec = ensure_job_ll_master(jobs, idx) if idx in jobs.index else (_jll := get_ll_for_address(job.get("address","")), _jll[0], _jll[1], classify_sector(_jll[0], _jll[1]))[1:]
                            if not sector_compatible(_tech_sector.get(t, "UNK"), jsec): continue
                            addr = job["address"]
                            ...
                            if jobs_count[t] > 0 and int(max_onsite_today) < int(MIN_ONSITE_CHUNK_MIN): continue
                            onsite_today_candidate = choose_onsite_no_crumbs(jm, max_onsite_today, MIN_ONSITE_CHUNK_MIN)
                            if onsite_today_candidate <= 0: continue
                            if best_long_cost is None or int(tmin) < best_long_cost:
                                best_long_idx = idx; best_long_cost = int(tmin); best_long_is_overtime = bool(is_overtime_candidate)
                        if best_long_idx is None: continue
                        job = jobs.loc[best_long_idx] if best_long_idx in jobs.index else solo_jobs.loc[best_long_idx]
                        base_job_id = str(job["job_id"]); jm_total = int(job["job_minutes"])
                        if bool(best_long_is_overtime):
                            ...
                            planned_rows.append({
                                ...
                                "job_min": int(jm_total), "buffer_min": int(buffer_job),
                        ...
                        carryover_by_tech[t] = {
                            ...
                            "techs_needed": int(job.get("techs_needed", 1)),
                            "total_parts": int(total_parts_guess), "part_idx_next": 1,
                            ...
                            "all_open_work": job.get("all_open_work", ""),
                        }
            ...
            for t in tech_names:
                if jobs_count[t] > 0:
                    ...
                    planned_rows.append({
                        "date": day.isoformat(), "technicien": t, "sequence": jobs_count[t] + 1,
                        "job_id": "RETURN_HOME", "cust": "", "duo": "", "ot": "",
                        "debut": mm_to_hhmm(int(used[t])), "fin": mm_to_hhmm(int(used[t]) + int(tback)),
                        "adresse": _home_map[t], "travel_min": int(tback), "job_min": 0,
                        "buffer_min": 0, "techs_needed": 1,
                        "last_inspection": "", "difference": "", "unit": "",
                        "serial_number": "", "all_open_work": "",
                        "description": "🏠 Retour domicile (estimé)",
                    })
                    used[t] = int(used[t]) + int(tback); cur_loc[t] = _home_map[t]

            if progress is not None:
                progress.progress(int(((di + 1) / total_steps) * 100))
            if progress_text is not None:
                _t_day = round(time.time() - _t_day_start, 1)
                progress_text.write(f"Planification… {di+1}/{len(month_days)} jour(s) — jour actuel: {_t_day}s | API calls: {st.session_state.get('p2_api_calls',0)} | cache hits: {st.session_state.get('p2_cache_hits',0)}")
        ...
        for t, stt in carryover_by_tech.items():
            ...
            if rem > 0:
                carryover_rows.append({
                    "job_id": str(stt.get("base_job_id", "")), "cust": str(stt.get("cust", "")),
                    "address": str(stt.get("address", "")), "description": str(stt.get("description", "")),
                    "job_minutes": rem, "techs_needed": int(stt.get("techs_needed", 1)),
                    ...
                    "all_open_work": str(stt.get("all_open_work", "")),
                })
        ...
        if not remaining_out.empty:
            ...
            for i, r in remaining_out.iterrows():
                if time.time() - _ot_flag_start > 10: break
                try:
                    addr = str(r.get("address", "")).strip()
                    jm = int(r.get("job_minutes", 0))
                    if not addr or jm <= 0: continue
                    ...
                    if ck in _best_need_cache:
                        ...
                        for t in candidates:
                            ...
                            conn = _get_db(); cur_db = conn.cursor()
                            now = int(time.time()); min_ts = now - int(cache_days) * 86400
                            cur_db.execute("SELECT minutes FROM travel WHERE k=? AND ts>=?", (k_fwd, min_ts))
                            row_fwd = cur_db.fetchone()
                            cur_db.execute("SELECT minutes FROM travel WHERE k=? AND ts>=?", (k_bck, min_ts))
                            row_bck = cur_db.fetchone()
                            if not row_fwd or not row_bck: continue
                            need = int(row_fwd[0]) + jm + int(buffer_job) + int(row_bck[0])
                            if best_need is None or need < best_need: best_need = need
        ...
        if not remaining_out.empty and month_days:
            ...
            for row in planned_rows:
                dk = str(row.get("date","")).strip(); tk = str(row.get("technicien","")).strip()
                if not dk or not tk: continue
                if str(row.get("job_id","")).upper() == "RETURN_HOME": continue
            ...
            backfill_booked = set(); backfill_rows = []
            ...
            for _, jrow in remaining_solo.iterrows():
                ...
                if jid in backfill_booked: continue
                if normalize_base_job_id(jid) in planned_base_ids: continue
                ...
                for day in month_days:
                    ...
                    for t in tech_names:
                        ...
                        if not fits_normal and not fits_ot: continue
                        # FIX 3 : travel_min_estimate (SQLite → haversine, jamais d'appel API)
                        # On évite travel_min_cached ici car le backfill itère sur
                        # jobs × jours × techs → risque de milliers d'appels Distance Matrix.
                        # L'estimation haversine est suffisante pour l'heure de début affichée.
                        tmin_real = travel_min_estimate(home_addr, addr, _t_blat, _t_blon, jlat, jlon)
                        tback_real = travel_min_estimate(addr, home_addr, jlat, jlon, _t_blat, _t_blon)
                        start_m = cur_used + int(tmin_real)
                        end_m = start_m + int(jm) + int(buffer_job)
                        ot_flag = "🟥 OT" if fits_ot and not fits_normal else ""
                        new_row = {
                            "date": dk, "technicien": t, "sequence": cur_count + 1,
                            "job_id": jid, "cust": str(jrow.get("cust","")),
                            "duo": "", "ot": ot_flag,
                            "debut": mm_to_hhmm(int(start_m)), "fin": mm_to_hhmm(int(end_m)),
                            "adresse": addr, "travel_min": int(tmin_real), "job_min": int(jm),
                            "buffer_min": int(buffer_job), "techs_needed": int(jrow.get("techs_needed", 1)),
                            ...
                            "all_open_work": str(jrow.get("all_open_work","")),
            ...
            if backfill_rows:
                ...
                remaining_out = remaining_out[~remaining_out["job_id"].astype(str).isin(backfill_booked)].copy()
                if progress_text is not None:
        ...
        try:
            ...
            if missing:
                if "ot_impossible" not in remaining_out.columns: remaining_out["ot_impossible"] = False
                if "missing_reinjected" not in remaining_out.columns: remaining_out["missing_reinjected"] = False
                missing_rows = []
                for mid in missing:
                    src = jobs_in[jobs_in["job_id"].astype(str) == str(mid)]
                    if src.empty: continue
                    rr = src.iloc[0].to_dict()
                    rr["missing_reinjected"] = True; rr["ot_impossible"] = False
        ...
        _timing_summary = {"concat_s": _t_concat, "ot_flag_s": _t_ot, "safety_s": _t_safety, "total_post_s": round(time.time() - _t_post, 1)}
        if progress_text is not None:
            progress_text.write(f"⏱️ Safety net: {_t_safety}s — returning…")
        success = remaining_out.empty and (len(carryover_rows) == 0)
        return {"success": bool(success), "rows": planned_rows, "remaining": remaining_out, "timing": _timing_summary}
    def _is_return_home(job_id): return str(job_id).strip().upper() == "RETURN_HOME"
    def _is_split_part(job_id): return "(PART" in str(job_id).upper()
    def _is_duo_row(row): return str(row.get("duo", "")).strip() != ""
    def _is_ot_row(row): return str(row.get("ot", "")).strip() != ""
    def _hhmm_to_mm(hhmm):
        try:
            s = str(hhmm or "").strip()
            if not s: return 0
            h, m = s.split(":")
    ...
    def optimize_route_ortools(tech, jobs_list):
        if not ORTOOLS_AVAILABLE or len(jobs_list) <= 1: return jobs_list
        home_addr = home_map[tech]
        all_locs = [home_addr] + [jb["adresse"] for jb in jobs_list] + [home_addr]
        n = len(all_locs)
        matrix = []; has_missing = False
        _locs_ll = [get_ll_for_address(_loc) if _loc else (None, None) for _loc in all_locs]
        for i in range(n):
            row_m = []
            for j in range(n):
                if i == j: row_m.append(0)
                    ...
                    row_m.append(int(v))
            matrix.append(row_m)
        if has_missing: return jobs_list
        try:
            ...
            if not solution: return jobs_list
            order = []
            index = routing.Start(0)
            while not routing.IsEnd(index):
                node = manager.IndexToNode(index)
                if 0 < node < n - 1: order.append(node - 1)
                index = solution.Value(routing.NextVar(index))
            if len(order) != len(jobs_list): return jobs_list
            return [jobs_list[i] for i in order]
        except Exception:
            return jobs_list

    def rebuild_day_greedy(day, tech, jobs_list, day_available_min, buffer_job, max_jobs_per_day, _travel_fn=None):
        home_addr = home_map[tech]
        cur = home_addr; used = 0; seq = 0
        _tfn = _travel_fn if _travel_fn is not None else travel_min_estimate
        optimized = optimize_route_ortools(tech, list(jobs_list)) if _travel_fn is None else list(jobs_list)
        remaining = list(optimized)
        out_rows: List[Dict[str, Any]] = []
        if len(remaining) > int(max_jobs_per_day): return None
        while remaining:
            best_i = None; best_tmin = None
            for i, jb in enumerate(remaining):
                tmin = _tfn(cur, jb["adresse"])
                if best_tmin is None or int(tmin) < int(best_tmin):
                    best_tmin = int(tmin); best_i = i
            jb = remaining.pop(best_i)
            tmin = int(best_tmin); tback = int(_tfn(jb["adresse"], home_addr))
            need = tmin + int(jb["job_min"]) + int(buffer_job) + tback
            if used + need > int(day_available_min): return None
            seq += 1; start_m = used + tmin; end_m = start_m + int(jb["job_min"]) + int(buffer_job)
            out_rows.append({
                ...
                "all_open_work": jb.get("all_open_work", ""),
                "description": jb.get("description", ""),
            })
            used = int(end_m); cur = jb["adresse"]
            if seq >= int(max_jobs_per_day) and remaining: return None
        if seq > 0:
            tback = int(_tfn(cur, home_addr))
            out_rows.append({
                "date": day, "technicien": tech, "sequence": seq + 1,
                "job_id": "RETURN_HOME", "cust": "", "duo": "", "ot": "",
                "debut": mm_to_hhmm(int(used)), "fin": mm_to_hhmm(int(used) + int(tback)),
                "adresse": home_addr, "travel_min": int(tback), "job_min": 0,
                "buffer_min": 0, "techs_needed": 1, "unit": "", "serial_number": "",
                "difference": "", "last_inspection": "", "all_open_work": "",
                "description": "🏠 Retour domicile (estimé)",
            })
        return out_rows

    def rebuild_day_in_order(day, tech, jobs_list, day_available_min, buffer_job, max_jobs_per_day, _travel_fn=None):
        home_addr = home_map[tech]
        cur = home_addr; used = 0; seq = 0; out = []
        _tfn2 = _travel_fn if _travel_fn is not None else travel_min_estimate
        if len(jobs_list) > int(max_jobs_per_day): return None
        for jb in jobs_list:
            tmin = int(_tfn2(cur, jb["adresse"])); tback = int(_tfn2(jb["adresse"], home_addr))
            need = tmin + int(jb["job_min"]) + int(buffer_job) + tback
            if used + need > int(day_available_min): return None
            seq += 1; start_m = used + tmin; end_m = start_m + int(jb["job_min"]) + int(buffer_job)
            out.append({
            ...
            used = int(end_m); cur = jb["adresse"]
            if seq >= int(max_jobs_per_day) and (len(out) < len(jobs_list)): return None
        if seq > 0:
    ...
    def repair_month_plan(planned_rows, tech_names, day_available_min, buffer_job,
                          max_jobs_per_day, threshold_travel_min=50, candidate_techs_top_n=4,
                          max_moves=25, timeout_seconds=30):
        if not planned_rows: return planned_rows, {"moves": 0, "attempts": 0, "improved": 0}
        ...
        for r in planned_rows:
            day_k = str(r.get("date","")).strip(); tech_k = str(r.get("technicien","")).strip()
            if _is_return_home(r.get("job_id","")): original_return_rows[(day_k, tech_k)] = r
        ...
        for r in movable:
            ...
            by_day_tech.setdefault(k, []).append({
                "job_id": r["job_id"], "cust": r.get("cust",""), "ot": r.get("ot",""),
                "adresse": r["adresse"], "job_min": int(r.get("job_min",0)),
                "techs_needed": int(r.get("techs_needed",1)), "description": r.get("description",""),
                "unit": r.get("unit",""), "serial_number": r.get("serial_number",""),
                "difference": r.get("difference",""), "last_inspection": r.get("last_inspection",""),
                "all_open_work": r.get("all_open_work",""),
        ...
        _modified_pairs: set = set()
        for (tmin_orig, day, src_tech, jb) in candidates:
            if time.time() - _repair_start > float(timeout_seconds): stats["timeout"] = True; break
            if stats["moves"] >= int(max_moves): break
            if int(tmin_orig) <= int(threshold_travel_min): break
            stats["attempts"] += 1
            ...
            if not near_techs: continue
            best_move = None
            for dst_tech in near_techs:
                src_key = (day, src_tech); dst_key = (day, dst_tech)
                ...
                if len(dst_list2) > int(max_jobs_per_day): continue
                ...
                if rebuilt_src is None or rebuilt_dst is None: continue
                new_tmin = None
                for rr in rebuilt_dst:
                    if str(rr.get("job_id")) == str(jb["job_id"]):
                        new_tmin = int(rr.get("travel_min", 9999)); break
                if new_tmin is None: continue
            ...
            _modified_pairs.add((day, src_tech)); _modified_pairs.add((day, dst_tech))
            stats["moves"] += 1; stats["improved"] += 1
        final_rows: List[Dict[str,Any]] = []
        for r in locked:
            if _is_return_home(r.get("job_id","")): continue
            final_rows.append(r)
        for (day, tech), lst in by_day_tech.items():
            if (day, tech) in _modified_pairs:
                ...
                if rebuilt is not None:
                    ...
                    if orig:
                        ...
                        if rebuilt2 is not None: final_rows.extend(rebuilt2)
                        else: final_rows.extend(orig)
            else:
                orig = original_rows_by_day_tech.get((day, tech), [])
        ...
        for r in final_rows:
            day_k = str(r.get("date","")).strip(); tech_k = str(r.get("technicien","")).strip()
            if not day_k or not tech_k: continue
        ...
        for (day_k, tech_k) in sorted(all_pairs):
            ...
            final_rows.append({
                "date": day_k, "technicien": tech_k, "sequence": int(last_row.get("sequence",0)) + 1,
                "job_id": "RETURN_HOME", "cust": "", "duo": "", "ot": "",
                "debut": mm_to_hhmm(int(used_end)), "fin": mm_to_hhmm(int(used_end) + int(tback)),
                "adresse": home_addr, "travel_min": int(tback), "job_min": 0,
                "buffer_min": 0, "techs_needed": 1, "unit": "", "serial_number": "",
                "difference": "", "last_inspection": "", "all_open_work": "",
                "description": "🏠 Retour domicile (estimé)",
    ...
    def build_export_excel(df: pd.DataFrame, unplanned_df: pd.DataFrame = None) -> bytes:
        ...
        def date_color(date_str):
            ...
            for fmt in ("%d-%b-%Y", "%Y-%m-%d", "%d/%m/%Y", "%m/%d/%Y"):
                try:
                    ...
                    diff = (dt - today).days
                    if diff < -180:  return "FF7B4A2D", "FFFFFF"
                    elif diff < 120: return "FFCCCCCC", "000000"
                    elif diff < 150: return "FFFFB6C1", "000000"
                    elif diff < 180: return "FFFFA500", "000000"
        ...
        def truncate_description(val, max_instances=4):
            ...
            result = " // ".join(kept)
            if len(parts) > max_instances: result += f" … (+{len(parts)-max_instances} more)"
            return result
        ...
        col_labels = {
            ...
            "all_open_work": "All Open Work",
            "insp_wo": "Insp. WO#", "insp_type": "Insp. Type", "insp_date": "Insp. Date",
            "job_minutes": "Durée (min)", "postal": "Code postal", "ot_impossible": "OT impossible",
            "address": "Adresse",
        }
        ...
        def write_sheet(ws, data_df, is_planning=True, row_offset=0):
            ...
            for ri, (_, row) in enumerate(out.iterrows(), 2 + row_offset):
                ...
                if is_return_home:   row_bg, row_fg = "FFF0F0F0", "888888"
                elif is_ot:          row_bg, row_fg = "FFF8D7DA", "000000"
                elif is_duo:         row_bg, row_fg = "FFD4EDDA", "000000"
                elif ri % 2 == 0:    row_bg, row_fg = "FFFAFAFA", "000000"
                else:                row_bg, row_fg = "FFFFFFFF", "000000"
                ...
                for ci, col in enumerate(cols, 1):
                    ...
                    cell.border = border
                    # wrap_text pour description ET all_open_work
                    cell.alignment = Alignment(vertical="center", wrap_text=(col in ("description","all_open_work")))
    ...
    def render_integrity_check_bookable(initial_ids_set: set):
        ...
        if st.session_state.get(cache_key + "_sig") != current_sig:
            ...
            for r in st.session_state.get("planning_month_remaining_rows", []):
                jid = r.get("job_id","")
                if jid: remaining_jobs.add(normalize_base_job_id(jid))
    ...
    mode = st.radio(
        "Choisir un mode",
        ["1 journée / 1 technicien (mode actuel)", "Mois complet — techniciens choisis par l'utilisateur", "Mois complet — techniciens choisis automatiquement"],
    ...
    def _dedup_rows(rows):
        ...
        for _r in rows:
            ...
            if _jid.upper() == "RETURN_HOME": continue
            ...
            if _key not in seen: seen.add(_key); out.append(_r)
        active_dt = set((_r.get("date",""), _r.get("technicien","")) for _r in out)
        for _r in rows:
            if str(_r.get("job_id","")).upper() == "RETURN_HOME":
                if (_r.get("date",""), _r.get("technicien","")) in active_dt:
                    out.append(_r)
        return out

    def _show_month_results(mode_key, month_df, tech_names_used, buffer_job_val):
        if st.session_state.get("planning_month_success"):
            st.success(f"Mois complété ✅ | Techs utilisés: {len(tech_names_used)}")
        else:
        ...
        if remaining_rows:
            ...
            if "ot_impossible" in unplanned_df.columns:
                ...
                if ot_imp > 0: st.warning(f"⚠️ Jobs impossibles même en OT (14h cap): {ot_imp}")
            if "techs_needed" in unplanned_df.columns:
                duo_left = int((unplanned_df["techs_needed"].astype(int) == 2).sum())
                if duo_left > 0: st.warning(f"⚠️ Jobs DUO restants (techs_needed=2): {duo_left}")
        ...
        st.download_button(
            label="⬇️ Télécharger le planning (.xlsx)", data=_excel_bytes,
    ...
    def _preferred_cols(df):
        preferred = ["date","technicien","sequence","job_id","cust","duo","ot","debut","fin","adresse",
                     "travel_min","job_min","buffer_min","techs_needed","unit","serial_number",
                     "all_open_work","last_inspection","difference","description"]
        return [c for c in preferred if c in df.columns] + [c for c in df.columns if c not in preferred]

    def _save_result(result, mode_key, chosen_techs):
        _rows = _dedup_rows(result["rows"])
        st.session_state["planning_month_rows"] = _rows
        st.session_state["planning_month_success"] = result["success"]
        st.session_state["planning_month_mode"] = mode_key
        st.session_state["planning_month_techs_used"] = chosen_techs
        remaining_df = result["remaining"].copy()
        cols_show = ["job_id","cust","address","description","job_minutes","techs_needed","postal","all_open_work","ot_impossible"]
    ...
    if mode == "1 journée / 1 technicien (mode actuel)":
        ...
        with c1: day_hours = st.number_input("Heures/jour", 4.0, 14.0, 8.0, 0.5, key="p2_day_hours")
        with c2: lunch_min = st.number_input("Pause (min)", 0, 120, 30, 5, key="p2_lunch")
        with c3: buffer_job = st.number_input("Buffer/job (min)", 0, 60, 10, 5, key="p2_buffer")
        with c4: max_jobs = st.number_input("Max jobs/jour", 1, 25, 10, 1, key="p2_max_jobs")
        only_one = st.checkbox("Filtrer: seulement jobs à 1 technicien", value=False, key="p2_only_one")
        service_choice = st.selectbox("Type de jobs", ["Inclure full service (tous les jobs)", "Generator inspection seulement", "Exclure generator inspection"], index=0, key="p2_service_choice")
        run = st.button("🚀 Générer la journée", type="primary", key="p2_run")
        if run:
            st.session_state["p2_api_calls"] = 0; st.session_state["p2_cache_hits"] = 0
            available = int(round(day_hours * 60)) - int(lunch_min)
            daily_onsite_cap = int(available)
            OT_ACTIVE_CAP = int(round(14 * 60)) - int(lunch_min)
            if OT_ACTIVE_CAP < available: OT_ACTIVE_CAP = available
            MIN_ONSITE_CHUNK_MIN = 180
            remaining = jobs.copy()
            remaining = filter_by_service_type(remaining, service_choice)
            if only_one: remaining = remaining[remaining["techs_needed"] <= 1].copy()
            remaining = remaining.sort_values(["job_id"], kind="mergesort")
            used = 0; seq = 0; cur_loc = home_addr; day_rows = []; carryover = None
            tsec = tech_sector_map.get(chosen_tech, "UNK")
            while True:
                if remaining.empty or seq >= int(max_jobs): break
                if carryover is not None and int(carryover.get("remaining_job_min", 0)) > 0: break
                best_idx = None; best_cost = None; best_tmin = None
                ...
                if best_idx is not None:
                    ...
                    seq += 1; start_m = used + int(best_tmin); end_m = start_m + int(job["job_minutes"]) + int(buffer_job)
                    day_rows.append({
                        ...
                        "last_inspection": job.get("last_inspection",""), "difference": job.get("difference",""),
                        "unit": job.get("unit",""), "serial_number": job.get("serial_number",""),
                        "all_open_work": job.get("all_open_work",""),
                    })
                    used = int(end_m); cur_loc = job["address"]
                ...
                if seq == 0:
                    ...
                    if best_ot_idx is not None:
                        ...
                        seq += 1; start_m = used + int(best_ot_tmin); end_m = start_m + int(job["job_minutes"]) + int(buffer_job)
                        day_rows.append({
                            "technicien": chosen_tech, "sequence": seq,
                            "job_id": job["job_id"], "cust": job.get("cust",""),
                            "duo": "", "ot": "🟥 OT",
                ...
                carryover = {
                    "base_job_id": base_job_id, "cust": job.get("cust",""),
                    "address": job["address"], "description": job["description"],
                    "techs_needed": int(job.get("techs_needed",1)), "total_parts": int(total_parts),
                    "part_idx_next": 1, "remaining_job_min": int(job["job_minutes"]),
                    "all_open_work": job.get("all_open_work",""),
                }
                remaining = remaining[remaining["job_id"] != job["job_id"]].copy()
                tmin = best_long["tmin"]; tback = best_long["tback"]
                max_onsite_today = int(available) - int(used) - int(tmin) - int(buffer_job) - int(tback)
                onsite_today = choose_onsite_no_crumbs(int(carryover["remaining_job_min"]), int(max_onsite_today), MIN_ONSITE_CHUNK_MIN)
                if onsite_today <= 0: break
                seq += 1; start_m = int(used) + int(tmin); end_m = start_m + int(onsite_today) + int(buffer_job)
                day_rows.append({
                    "technicien": chosen_tech, "sequence": seq,
                ...
                carryover["part_idx_next"] = 2; break
        ...
        if day_rows_saved:
            ...
            preferred = ["technicien","sequence","job_id","cust","duo","ot","debut","fin","adresse",
                         "travel_min","job_min","buffer_min","techs_needed","unit","serial_number",
                         "all_open_work","difference","last_inspection","description"]
            ...
            st.write(f"**Total actif:** {total_travel+total_job+total_buffer} / {available_active} min")
            st.subheader("🧾 Coûts (estimation)")
            st.write(f"**Appels API (cette run):** {st.session_state.get('p2_api_calls',0)}")
            st.write(f"**Hits cache (cette run):** {st.session_state.get('p2_cache_hits',0)}")
            st.subheader("🧩 Jobs non planifiés")
    ...
    # ════════════════════════════════════════════════════════════════
        ...
        chosen_techs = st.multiselect("Choisir les techniciens", options=tech_names_all, default=st.session_state.get("p2_month_fixed_techs",[]), key="p2_month_fixed_techs")
        c1, c2, c3, c4 = st.columns(4)
        with c1: day_hours_m = st.number_input("Heures/jour", 4.0, 14.0, 8.0, 0.5, key="p2m_day_hours")
        with c2: lunch_min_m = st.number_input("Pause (min)", 0, 120, 30, 5, key="p2m_lunch")
        with c3: buffer_job_m = st.number_input("Buffer/job (min)", 0, 60, 10, 5, key="p2m_buffer")
        with c4: max_jobs_m = st.number_input("Max jobs/jour/tech", 1, 25, 10, 1, key="p2m_max_jobs")
        ...
        if run_month:
            ...
            if len(chosen_techs) == 0:
                ...
                if allow_duo and (jobs["techs_needed"] == 2).any() and len(chosen_techs) < 2:
                    st.error("Il y a des jobs DUO (2 techs), mais tu as sélectionné moins de 2 techniciens."); st.stop()
                days = business_days(month_start, month_end)
                progress = st.progress(0); progress_text = st.empty()
                ...
                _save_result(result, "fixed", chosen_techs)
                progress.progress(100); progress_text.write("Terminé ✅")
                timing = result.get("timing", {})
                if timing:
                    st.info(f"⏱️ **Post-planification** — Concat: {timing.get('concat_s','?')}s | OT-flag: {timing.get('ot_flag_s','?')}s | Safety net: {timing.get('safety_s','?')}s | **Total post: {timing.get('total_post_s','?')}s**")
        ...
        if month_rows_saved and st.session_state.get("planning_month_mode") == "fixed":
            ...
            month_df = month_df[_preferred_cols(month_df)]
            _show_month_results("fixed", month_df, techs_used, st.session_state.get("p2m_buffer",10))
    ...
    # ════════════════════════════════════════════════════════════════
        ...
        c1, c2, c3, c4 = st.columns(4)
        with c1: day_hours_m = st.number_input("Heures/jour", 4.0, 14.0, 8.0, 0.5, key="p2a_day_hours")
        with c2: lunch_min_m = st.number_input("Pause (min)", 0, 120, 30, 5, key="p2a_lunch")
        with c3: buffer_job_m = st.number_input("Buffer/job (min)", 0, 60, 10, 5, key="p2a_buffer")
        with c4: max_jobs_m = st.number_input("Max jobs/jour/tech", 1, 25, 10, 1, key="p2a_max_jobs")
        ...
        if run_month:
            st.session_state["p2_api_calls"] = 0; st.session_state["p2_cache_hits"] = 0
            days = business_days(month_start, month_end)
            outer_progress = st.progress(0); outer_text = st.empty()
            has_duo_jobs = allow_duo and (jobs["techs_needed"] == 2).any()
            lo = 2 if has_duo_jobs else 1; hi = len(tech_names_all); best = None
            def _run_k(k):
            ...
            outer_text.write("Recherche du nombre optimal de techniciens (binary search)…")
            iterations = 0
            while lo < hi:
                mid = (lo + hi) // 2
                outer_text.write(f"Essai avec {mid} technicien(s)… (recherche {lo}–{hi})")
                result, chosen = _run_k(mid)
                best = {"rows": result["rows"], "success": result["success"], "remaining": result["remaining"], "techs_used": chosen}
            ...
            _save_result(best, "auto", best["techs_used"])
        ...
        if month_rows_saved and st.session_state.get("planning_month_mode") == "auto":
            ...
            month_df = month_df.sort_values(sort_cols, ascending=True).reset_index(drop=True)
            month_df = month_df[_preferred_cols(month_df)]
            _show_month_results("auto", month_df, techs_used, st.session_state.get("p2a_buffer",10))

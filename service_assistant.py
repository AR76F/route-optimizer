...
try:
    ...
    PRIORITIZATION_ENGINE_AVAILABLE = True
    _PRIORITIZATION_IMPORT_ERROR = None
except Exception as exc:
    PRIORITIZATION_ENGINE_AVAILABLE = False
    _PRIORITIZATION_IMPORT_ERROR = exc
    determine_priority = None
    explain_priority = None
    compare_jobs = None
    higher_priority_job = None
...
def get_service_prioritization(job_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Safe wrapper around prioritization_engine.py.

    Returns a dictionary so service_assistant.py can keep working even if the
    engine is missing, broken, or not yet fully wired.
    """
    if not PRIORITIZATION_ENGINE_AVAILABLE:
        return {
            "available": False,
            "priority": None,
            "client_class": None,
            "schedule_code": None,
            "compact_schedule_code": None,
            "reason": "Prioritization engine unavailable.",
            "explanation": "Prioritization engine unavailable.",
            "missing_info": [],
            "decision_path": [],
            "needs_human_review": True,
            "error": str(_PRIORITIZATION_IMPORT_ERROR) if _PRIORITIZATION_IMPORT_ERROR else None,
        }

    try:
        result = determine_priority(job_data)

        return {
            "available": True,
            "priority": result.priority,
            "client_class": result.client_class,
            "schedule_code": result.schedule_code,
            "compact_schedule_code": result.compact_schedule_code,
            "reason": result.reason,
            "explanation": explain_priority(job_data),
        ...
        return {
            ...
            "schedule_code": None,
            "compact_schedule_code": None,
            "reason": "Prioritization engine failed.",
            "explanation": f"Prioritization engine failed: {exc}",
            "missing_info": [],
            "decision_path": [],
            "needs_human_review": True,
            "error": str(exc),
...
def append_prioritization_block(response_text: str, job_data: Dict[str, Any]) -> str:
    """
    Optional helper to append the priority result to an existing assistant reply.
    """
    priority = get_service_prioritization(job_data)

    if not priority["available"]:
        return response_text + "\n\n[Prioritization unavailable]"

    block = (
        "\n\n### Dispatch Priority\n"
        f"- Priority: {priority['priority']}\n"
        f"- Client class: {priority['client_class']}\n"
        f"- Schedule code: {priority['schedule_code']}\n"
        f"- Reason: {priority['reason']}\n"
    )

    if priority["missing_info"]:
        block += f"- Missing info: {', '.join(priority['missing_info'])}\n"

    if priority["needs_human_review"]:
        block += "- Human review recommended.\n"
...
PRIORITIZATION_HINTS = {
    "priority", "prioritize", "dispatch", "outage", "outage/no-redundancy",
    "alarm", "blocking", "reliability", "commissioning", "preventive maintenance",
    "pm", "generator down", "unit down", "ats", "schedule code", "p1", "p2",
    "p3", "p4", "p5", "p6",
}

PRIORITIZATION_EXTRACTION_MODEL = "gpt-4o-mini"


def is_prioritization_request(question: str) -> bool:
    q = normalize_text(question)
    return any(hint in q for hint in PRIORITIZATION_HINTS)


def extract_prioritization_payload(
    question: str,
    temporary_context: str | None = None,
    uploaded_sources: Iterable[Any] | None = None,
) -> dict[str, Any] | None:
    """
    Stage 2: ask GPT to extract only the fields needed by determine_priority().
    ...
    extraction_prompt = f"""
Extract the dispatch/prioritization fields from the user's message.

Return JSON only. No markdown. No commentary.

Allowed keys:
customer_class, client_rank, commissioning, is_pm, is_service_call,
unit_available, alarm_level, no_redundancy, health_safety_risk,
pm_move_count, pm_type, tech_count, p1_breakdown_waiting,
request_age_hours, non_urgent,
is_government, is_datacenter, is_hospital, is_critical_infrastructure,
is_telecom, has_maintenance_contract, is_residential, notes

Rules:
- Use C1, C2, C3, C4 for customer_class when known.
- Use true / false / null for booleans.
- Use null when unknown.
- Keep notes brief.
- If the input is not about dispatch priority, still return JSON with fields set to null where appropriate.
...
""".strip()
    ...
    try:
        response = OPENAI_CLIENT.responses.create(
            model=PRIORITIZATION_EXTRACTION_MODEL,
            input=[
                {
                    "role": "user",
                    "content": [
        ...
        # Best effort JSON cleanup in case the model wraps extra whitespace.
        start = raw.find("{")
        end = raw.rfind("}")
...
def format_priority_response(priority: dict[str, Any], original_question: str) -> str:
    """
    Stage 3 output formatting.
    """
    if not priority.get("available"):
        return (
            "I could not determine a dispatch priority automatically.\n\n"
            f"Reason: {priority.get('reason', 'Unknown')}"
        )

    lines = [
        f"Priority: {priority.get('priority')}",
        f"Client class: {priority.get('client_class')}",
        f"Schedule code: {priority.get('schedule_code')}",
        f"Reason: {priority.get('reason')}",
    ]

    missing = priority.get("missing_info") or []
    if missing:
        lines.append(f"Missing info: {', '.join(missing)}")

    if priority.get("needs_human_review"):
        lines.append("Human review recommended.")

    decision_path = priority.get("decision_path") or []
    if decision_path:
        lines.append("Decision path: " + " -> ".join(decision_path))
...
def ask_service_assistant(
    ...
    """
    # Stage 1: detect whether this is a prioritization request
    if is_prioritization_request(question):
        # Stage 2: extract structured fields into JSON
        payload = extract_prioritization_payload(
            question,
            temporary_context=temporary_context,
            uploaded_sources=uploaded_sources,
        )
        ...
        if payload is not None:
            # Stage 3: send JSON into the rule engine
            priority = get_service_prioritization(payload)
            return format_priority_response(priority, question)

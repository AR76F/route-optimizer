---
title: Company Acronyms - T
title_fr: Acronymes de l'entreprise - T
category: reference
subcategory: company_acronyms
region: Shared
language: bilingual
keywords:
  - company acronyms
  - acronym reference
  - acronym T
  - acronymes de l'entreprise
  - référence des acronymes
  - acronymes T
---

# Company Acronyms - T

# Version 1.1.0

# Last Updated: 2026-07-14

# Updated By: Jipeng Li

# Change Log:

v1.1.0
- Added acronyms from CSSNA Field Service PowerGen Playbook.

v1.0.0
- Creation of the initial file.

## Purpose

This document contains company acronyms beginning with the letter specified in the title.

## Acronyms

| Acronym | Definition |
|---------|------------|
| T&A | Time and Attendance |
| T&R | Troubleshooting and Repair |
| T/S | Troubleshooting |
| T/WHL | Turbine Wheel |
| T2B5 | Tier 2, Bin 5 |
| TAF | Transaction Assessment Form |
| TAHR | Transient Air Handling Regulator |
| TAL | Time at level |
| TAL | Technical Area Leader |
| TAM | Total Addressable Market |
| TAP | Technician Apprentice Program |
| TBA | to be announced |
| TBV | Turbine Bypass Valve |
| TBWS | Team Based Work System |
| TCA | Trading Community Architecture |
| TCC | Total Cash Compensation |
| TCC | Technical Compliance & Certification |
| TCL | Tata Cummins Limited |
| TCLC | Tech Center Leaders Conference |
| TCO | Total Cost Ownership |
| TCP | Technology Control Plan |
| TCPD | Turbocompound |
| TCS | Tata Consultancy Services |
| TDC | Top Dead Center |
| TDE | Transparent Data Encryption |
| TDFSS | Technology Development for Six Sigma |
| TDOS | Torque Drop Off Speed |
| TDR | Temporary Deviation Request |
| TE | Transmission error |
| TE | Timing Extension |
| TE | Test Engineer |
| TE-DE | Technology-Design |
| TEA | Tiny Encryption Algorithm |
| TEC | Technical Education for Communities |
| TFE | Technical Functional Excellence |
| TFR | Too Frequent Regeneration |
| TFR | Technical Functional Review |
| TFSI | Turbocharged Fuel Stratified Injection |
| TGC | Turbospeed Governor Control |
| TH | Test Hours |
| TIR | Total Indicator Reading |
| TIS | Tool Interfeace Specification |
| TIS | Thermal Imaging System |
| TIS | Time in System |
| TISAX | Trusted Information Security Assessment Exchange |
| TIT | Turbine Inlet Temperature |
| TKE | Turbulent Kinetic Energy |
| TLA | Top Level Assembly |
| TLA | Three Letter Acronym |
| TLN | Top Level Number |
| TM | Thermal Management |
| TM | Team Manager |
| TM | Territory Manager |
| TMF | Thermo Mechanical Fatigue |
| TML | Tata Motors Limited |
| TMS | Thermal Management System |
| TMS | Talent Management System |
| TNF | Trouble Not Found |
| TOV | Turbine Outlet Volute |
| TP | Technical Profile |
| TP | Tail Pipe |
| TPEM | Transition Program for Equipment Manufacturers |
| TPIA | Trans Param Idle Adjust |
| TPL | Technical Project Leader |
| TPM | Total Productive Maintenance |
| TPP | Transform Project Plan |
| TPSE | Technical Product Support Engineer |
| TQA | Total Quality Assurance |
| TQC | Total Quality Control |
| TQM | Total Quality Management |
| TQS | Total Quality System |
| TRCAT | Transient Regeneration Capability Assessment Test |
| TRIC | Turbo Rotordynamic Internal Code |
| TRIZ | Russian for 'Theory of Inventive Problem Solving' |
| TRL | Technology Readiness Level |
| TRL | Torque Rate Limit |
| TRP | Temporary Repair Practice |
| TRR | Top Ring Reversal |
| TRV | Test Readiness Verification |
| TSB | Technical Service Bulletin |
| TSC | Torque Speed Control |
| TSC | Transit Shuttle Coach |
| TSC | Theoretical Sharp Corner |
| TSCA | Toxic Substances Control Act (US) |
| TSD | Turbo Speed Derate |
| TSFE | Thermal and Fluid Sciences Functional Excellence |
| TSL | Targeted Stocking Level |
| TSL | Turbo Speed Limit |
| TSM | Technical Support Manager |
| TSN | Test Specification Number |
| TSOB | Transaction Set of Books |
| TSP | Telematics Service Provider |
| TSR | Technical Service Report |
| TSR | Technical Support Request |
| TT | Ticketing Tool |
| TTIR | Turbine Thrust Influence Ratio |
| TTL | Test Team Leader |
| TTM | Tech Team Meeting and Time To Market |
| TTR | Test Trip Request |
| TTT | Top Tank Temperature |
| TTVM | Testing, Troubleshooing, and Voltage Measurement |
| TVA | Torsional Variation Analysis |
| TVP | Total Value of Performance |
| TWA | ​Tire Wear Adjustment |
| TWC | Three Way Catalyst |
| TWIG | Technical Work Integration Group |
| TWMS | Technical Work Management System |
---
title: Company Acronyms - T
title_fr: Acronymes de l'entreprise - T
category: reference
subcategory: company_acronyms
region: Shared
language: bilingual
keywords:
  - company acronyms
  - acronym reference
  - acronym T
  - acronymes de l'entreprise
  - référence des acronymes
  - acronymes T
---
---
title: Company Acronyms - T
title_fr: Acronymes de l'entreprise - T
category: reference
subcategory: company_acronyms
region: Shared
language: bilingual
keywords:
  - company acronyms
  - acronym reference
  - acronym T
  - acronymes de l'entreprise
  - référence des acronymes
  - acronymes T
---
---
title: Company Acronyms - T
title_fr: Acronymes de l'entreprise - T
category: reference
subcategory: company_acronyms
region: Shared
language: bilingual
keywords:
  - company acronyms
  - acronym reference
  - acronym T
  - acronymes de l'entreprise
  - référence des acronymes
  - acronymes T
---
